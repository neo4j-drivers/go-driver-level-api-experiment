...
type Driver interface {
	...
	// ExecuteQueryTransformResults runs the specified query and optional parameters in a retryable transaction function
	// The query will be re-executed until it is successful or a number of attempts has been reached
	// The result is mapped with the configured result transformer
	ExecuteQueryTransformResults(ctx context.Context, query string, params map[string]any, config QueryConfig) (any, error)
...
func (d *driver) ExecuteQuery(ctx context.Context, query string, params map[string]any, options ...QueryConfigOption) (_ *EagerResult, err error) {
	...
	queryConfig.ResultTransformer = transformToEagerResult
	result, err := d.ExecuteQueryTransformResults(ctx, query, params, queryConfig)
	if err != nil {
		return nil, err
	}
...
func (d *driver) ExecuteQueryTransformResults(ctx context.Context, query string, params map[string]any, queryConfig QueryConfig) (_ any, err error) {
	...
	return txFuncApi(ctx, RunQuery(ctx, query, params, queryConfig.ResultTransformer))
}
...
type ResultTransformer func(context.Context, neo4j.ResultWithContext) (any, error)
...
func WithResultTransformer(transformer ResultTransformer) QueryConfigOption {
	return func(config *QueryConfig) {
		config.ResultTransformer = transformer
	}
}
...
func RunQuery(ctx context.Context, query string, params map[string]any, transformer ResultTransformer) neo4j.ManagedTransactionWork {
...
func transformToEagerResult(ctx context.Context, result neo4j.ResultWithContext) (any, error) {
